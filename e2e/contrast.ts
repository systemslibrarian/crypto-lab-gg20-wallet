 ...
 *    files the node under `incomplete`. The wraparound number line's track is a
 *    three-stop `linear-gradient` from `--ok` through `--warn` to `--danger`,
 *    and its tick and marker labels sit directly on it.
 ...
 *    0.01ms rather than cancelling them, so end states are preserved.
 *
 * It also exists because the "gradient contrast check" this replaced measured
 * nothing at all: it read `var(--background)` and `var(--text)` off two
 * throwaway divs and compared those two values. `--background` is not a token
 * in this stylesheet, so it resolved to nothing and the helper's `parseRGB`
 * fallback compared the page text against `[0, 0, 0]`.
 *
 ...
 * thing as "painted underneath". The number line's labels are absolutely
 * positioned and routinely render outside the track they are anchored to, and
 * then the track's gradient is simply not behind them. So an ancestor's own
 * paint is applied only when its border box actually intersects the text's box;
 * a partial intersection still counts, so the judgement stays worst-case.
 * Opacity is unconditional either way — an opacity group fades its whole
 * subtree wherever that subtree happens to paint.
 *
 ...
 *    1024-bit Paillier moduli and secp256k1 points inside `.math` and
 *    `.table-wrap` scrollers with `min-width: 560px` tables, so at 380px much
 *    of that content is scrolled away. Content past the client box is not
 ...
 *    through to white, inventing a failure. Skip it, and rely on the wider
 *    viewport where the same element is measured for real.
 *
 ...
 *    draws no SVG figures — the party diagram and the number line are both
 *    plain elements — but the rule is kept because it is cheap and silent when
 ...
 *    not an ancestor. Note the underlay deliberately ignores stroke-only
 *    geometry such as `<line>`: SVG's initial `fill` is black and
 *    `getComputedStyle` reports that even for a line, so compositing one
 *    invents an opaque black backdrop under whatever it crosses.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here — and the gate this
     * replaced proved it, because that is exactly what it used. The shared
     * header mixes this lab's `--accent` into its surfaces with `color-mix()`,
     * which Chromium reports to `getComputedStyle` unchanged rather than
     * converting to sRGB. A
     ...
     *
     * This page renders eight exhibits of dense mono text — party-diagram cells,
     * verdict lines, truncated moduli — and every one of them walks the same
     * handful of ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has two: the symbols key and the honesty
      // notes. Their OPEN state is a real state the gate scans explicitly,
      // rather than being forced open before every scan the way the old gate
      // did.
      ...
      // `.skip-link` both park off the top edge and slide in only on focus.
      // Measuring the parked copy invents a failure for text that is not on
      // screen; the focused rendering is a real state the gate scans explicitly.
     ...
     * stricter than axe on decorative content. Honour the same boundary — for
     * the verdict glyphs here (✓ / ✗ / ⚠), whose meaning is repeated in the
     * words beside them. Note this narrowing is not a coverage hole: axe's own
     * `color-contrast` rule keys off VISUAL visibility rather than the
     * accessibility tree, so it still measures `aria-hidden` text that paints.
     */
     ...
     * screen but measurable by a naive walk. This lab has no SVG figures, so
